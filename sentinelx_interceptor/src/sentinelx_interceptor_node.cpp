  ...
  seeker_locked_(false)
{
  ...
  px4_state_sub_ = create_subscription<sentinelx_msgs::msg::PX4VehicleState>(
    "/sentinelx/px4/state", 10,
    std::bind(&SentinelXInterceptorNode::on_px4_state, this, std::placeholders::_1));

  seeker_status_sub_ = create_subscription<sentinelx_msgs::msg::SeekerStatus>(
    "/sentinelx/seeker/status", 10,
    std::bind(&SentinelXInterceptorNode::on_seeker_status, this, std::placeholders::_1));

  seeker_track_sub_ = create_subscription<sentinelx_msgs::msg::SeekerTrack>(
    "/sentinelx/seeker/track", 10,
    std::bind(&SentinelXInterceptorNode::on_seeker_track, this, std::placeholders::_1));

  guidance_pub_ = create_publisher<sentinelx_msgs::msg::GuidanceCommand>("/sentinelx/guidance/command", 10);
  phase_pub_ = create_publisher<sentinelx_msgs::msg::InterceptorPhase>("/sentinelx/interceptor/phase", 10);
  target_estimate_pub_ = create_publisher<sentinelx_msgs::msg::InternalTargetEstimate>("/sentinelx/interceptor/target_estimate", 10);
...
void SentinelXInterceptorNode::on_px4_state(const sentinelx_msgs::msg::PX4VehicleState::SharedPtr msg)
{
  latest_px4_state_ = *msg;
  has_px4_state_ = true;
}

void SentinelXInterceptorNode::on_seeker_status(const sentinelx_msgs::msg::SeekerStatus::SharedPtr msg)
{
...
void SentinelXInterceptorNode::on_seeker_track(const sentinelx_msgs::msg::SeekerTrack::SharedPtr msg)
{
...
{
  sentinelx_msgs::msg::GuidanceCommand msg;
  msg.stamp = now();
  ...
  if (phase_ == Phase::TerminalHoming && seeker_locked_) {
    msg.guidance_mode = sentinelx_msgs::msg::GuidanceCommand::GUIDANCE_SEEKER_TERMINAL;
    ...
    msg.guidance_mode = sentinelx_msgs::msg::GuidanceCommand::GUIDANCE_GPS_INS;
    msg.use_position = true;
    msg.target_x_m = 0.0F;
    msg.target_y_m = 0.0F;
    msg.target_z_m = -10.0F;
  } else if (phase_ == Phase::ReturnHome) {
    msg.guidance_mode = sentinelx_msgs::msg::GuidanceCommand::GUIDANCE_RETURN_HOME;
  } else {
    msg.guidance_mode = sentinelx_msgs::msg::GuidanceCommand::GUIDANCE_HOLD;
  }
...
{
  sentinelx_msgs::msg::InterceptorPhase msg;
  msg.stamp = now();
  msg.interceptor_id = interceptor_id_;
  msg.mission_id = mission_id_;
  msg.target_id = target_id_;
  ...
  msg.authority = msg.terminal_phase ? sentinelx_msgs::msg::InterceptorPhase::AUTHORITY_SEEKER : sentinelx_msgs::msg::InterceptorPhase::AUTHORITY_INERTIAL;
  phase_pub_->publish(msg);
}

void SentinelXInterceptorNode::publish_target_estimate()
{
  sentinelx_msgs::msg::InternalTargetEstimate msg;
  msg.stamp = now();
  msg.interceptor_id = interceptor_id_;
  msg.target_id = target_id_;
  msg.valid = has_c2_track_ || seeker_detected_;
  msg.source = seeker_locked_ ? sentinelx_msgs::msg::InternalTargetEstimate::SOURCE_SEEKER : sentinelx_msgs::msg::InternalTargetEstimate::SOURCE_C2_TRACK;
